...
Make sure that the household IDs are consistent between your *person* spreadsheet and your *household* spreadsheet. In there *household* spreadsheet, the IDs will never be repeated from one row to another, while in the *person* spreadsheet the IDs will be repeated as needed to indicate the household to which each person belongs.
...
The unit of observation in your *household* spreadsheet corresponds to one or more people living in a home, clinic, institution, and such. @fig-household-sheet-1 shows an example with the correct unit of observation, but with other items that need work. 
...
4. We don't need a family name in the *household* sheet. This is because often not everyone in a given household has the same last name. We can get the last name(s) from the *persons* sheet. 

5. Strictly speaking, we don't need a number-of-people column. This can easily be calculated from the *persons* sheet. Nevertheless, let's do include number-of-people in the *household* sheet. We can use it to check the internal consistency of the data.
